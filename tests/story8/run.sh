...
OUT_DIR="tests/story8/out"
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"

"$JAVAC" -d "$OUT_DIR" \
  sholi/src/main/java/name/soulayrol/rhaa/sholi/sync/credentials/*.java \
  sholi/src/main/java/name/soulayrol/rhaa/sholi/sync/settings/*.java \
  ...
  sholi/src/main/java/name/soulayrol/rhaa/sholi/sync/orchestration/*.java \
  tests/story8/src/*.java

"$JAVA" -cp "$OUT_DIR" story8.Story8TestRunner
